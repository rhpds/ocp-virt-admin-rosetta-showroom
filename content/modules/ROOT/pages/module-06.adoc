...
+
To be more percise, you can use *jq* to only return the *nodeName* and assign it to a variable for later use:
...
+
. Create a failure condition, access the node from step 4, using the *${node}* variable and *oc debug*, to put it into an unhealthy state by stopping the *kubelet* service.
...
[IMPORTANT]
====
Stop the *fencing-vm1* VM using *virtctl* from your *Terminal* window to ensure you have enough resources for the next labs.
