...
+
Further down on the same screen, leave *Min healthy* and both *Type* and *Status* dropdowns as the defaults.
...
+
. Once the *debug pod* exists, monitor the status of your Mirtual Machine, *fencing-vm1*. You will see that the self remediation process is isolating the node and migrating the workloads.
...
+
. In second terminal you can monitor the state of the nodes to watch your worker remediate.
