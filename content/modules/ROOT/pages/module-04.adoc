...
+
. Set an east node label on any worker where the *node-affinity-vm* is currently not running.
...
+
. Before we make *node-affinity-vm* move because of our new Node Affinity rule. Let's check its current location by looking at OpenShift Console and navigating to *Virtualization →  VirtualMachines → affinity* or check on the OpenShift Command line.
...
+
. Using the *node-affinity-vm* VM, Click on *Action → Stop*.
