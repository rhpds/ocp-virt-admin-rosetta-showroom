...
+
. See where the the *node-affinity-vm* is running by looking at OpenShift Console and navigating to *Virtualization →  VirtualMachines → affinity* or check on the OpenShift Command line.
...
node-affinity-vm   78m   Running   10.235.0.25   worker-cluster-t96sv-3   True
----

+
. Set a label `zone=east` on any worker where the *node-affinity-vm* is currently not running.
...
.Output
----
node/worker-cluster-t96sv-2 labeled
----
...
+
. To make the VM follow these new Node Affinity rules, you could migrate it to the node with the label. However, to show that this Node Affinity rule is working we are going to power off the VM completely and then power it back on. 

+
. Stop the the *node-affinity-vm* by clicking on *Action → Stop*.

+
. Once the VM completely stops. Click *Actions → Start*.

+
. The VM's relocation to the node with the assigned label is visible via the GUI by navigating to *node-affinity-vm → Overview*. Alternatively, you can verify this change using the OCP CLI.
...
----
NAME               AGE   PHASE     IP            NODENAME                 READY
node-affinity-vm   58s   Running   10.233.2.20   worker-cluster-t96sv-2   True
----
...
[source,text,role=execute]
----
oc get vmi pod-affinity-vm -n affinity
oc get vmi node-affinity-vm -n affinity
----
...
----
NAME              AGE   PHASE     IP            NODENAME                 READY
pod-affinity-vm   70m   Running   10.235.0.54   worker-cluster-t96sv-3   True
NAME               AGE     PHASE     IP            NODENAME                 READY
node-affinity-vm   3m33s   Running   10.233.0.46   worker-cluster-t96sv-2   True
----
...
----
NAME              AGE   PHASE     IP            NODENAME                 READY
pod-affinity-vm   70m   Running   10.235.0.54   worker-cluster-t96sv-2   True
NAME               AGE     PHASE     IP            NODENAME                 READY
node-affinity-vm   3m33s   Running   10.233.0.46   worker-cluster-t96sv-2   True
----
...
----
NAME                   AGE   PHASE     IP            NODENAME                 READY
pod-anti-affinity-vm   79m   Running   10.233.0.44   worker-cluster-t96sv-2   True
NAME               AGE   PHASE     IP            NODENAME                 READY
node-affinity-vm   13m   Running   10.233.0.46   worker-cluster-t96sv-2   True
----
...
----
NAME                   AGE   PHASE     IP            NODENAME                 READY
pod-anti-affinity-vm   79m   Running   10.233.0.44   worker-cluster-t96sv-3   True
NAME               AGE   PHASE     IP            NODENAME                 READY
node-affinity-vm   13m   Running   10.233.0.46   worker-cluster-t96sv-2   True
----
