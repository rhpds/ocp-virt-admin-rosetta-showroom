...
****
OpenShift Affinity Rules are similar to VMWare's Virtual Machine Affinity Rules.
If one were to also employ the OpenShift Descheduler for smart automated rescheduling of VMs, this feature would be similar to VMware's VM Affinity Rules coupled with the Distributed Resource Scheduler (DRS).
But that is not covered in this module.
****
...
+
. To make things permanent, use the OpenShift CLI to set a label `app: fedora` on the *node-affinity-vm*.
...
[.wrap,console,role=execute]
----
oc get vmi -n pod-affinity-vm affinity
----
