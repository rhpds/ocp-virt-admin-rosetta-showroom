...
The default _10:1_ overcommit ratio can be configured to the desired overcommit level by changing the *cpuAllocationRatio* on the *KubeVirt* Custom Resource. Changing this ratio will influence the CPU requests for each vCPU that is allocated by default, which enforces the maximum level of CPU overcommit through Kubernetes based request scheduling.
...
.Output
----
Now using project "over-commit" on server ...
----

+
[,shell,role="execute wrap",subs="attributes"]
----
virtctl start overcommit-vm-1
----
...
.Output
----
{"cores":16,"maxSockets":8,"model":"EPYC","sockets":2,"threads":1}
----

+
.. Using the *OpenShift Console*, navigate to *Virtualization → Virtual Machines* and select the *overcommit-vm-1* VM to view the *CPU/Memory* allocation.
...
+
IMPORTANT: In the following step, you need to replace "node_name" with an actual node from your cluster.
...
.Output
----
affinity           pod-anti-affinity-vm    4h19m   Running   10.133.2.11   worker-cluster-jlkcx-1  True    True
over-commit        overcommit-vm-1         47m     Running   10.133.2.25   worker-cluster-jlkcx-1   True    True
----
...
+
OpenShift Virtualization applies a default *10:1 CPU overcommit ratio* when you don't explicitly specify CPU requests. This means that if a VM has multiple vCPUs, the actual CPU request on the *virt-launcher* pod will be 1/10th of the total vCPUs of the VM.
...
+
Using the *OpenShift Console*, navigate to *Workloads → Pods* and select the *virt-launcher* pod for the VM.
...
IMPORTANT:  wasp-agent will be replaced by the native kubernetes based SWAP starting 4.21.
...
For example, with a VM with 16 GiB configured, if the overcommit percentage is set to its default value of 100, the memory request that CNV will apply to the pod is 16 GiB plus some extra for the overhead of the QEMU process running the VM.
If it's set to 200, the request will be set to 8 GiB plus the overhead.
...
+
NOTE: The virt-launcher pod is now requesting _1540Mi_ which is ~ _1.54GiB_.
