...
****
VMware admins typically interact with CPU and memory overcommit as part of a more collaborative relationship with Capacity Management teams, applying values *to each ESXi host* that are often provided to them by those teams or other 3rd party capacity management systems.
****
endif::[]
...
* <<cpu,Understand and Overcommit CPU on a Virtual Machine>>
* <<memory,Understand, Enable and Overcommit Memory on a Virtual Machine>>
...
+
.. Sum the vCPUs for all VMs on the node:
...
+
NOTE: You should at least see "32" but it may differ if more VMs are actually running on the same node.
...
Kubernetes, until 1.21, and OpenShift, until 4.21, did not have a Generally Available native SWAP implementation and thus did not allow use of SWAP.
...
[IMPORTANT]
====
To calculate the request value when overcommitting memory, you can use the following formula:
...
[IMPORTANT]
====
Stop the *overcommit-vm-1* VM using *virtctl* from your *Terminal* window to ensure you have enough resources for the next labs.
