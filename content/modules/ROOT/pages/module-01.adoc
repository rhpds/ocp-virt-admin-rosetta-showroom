...
VMware uses the concept of Datastores.
VMware Datastores can be thought of as LUNs, which may or may not belong to a Datastore Cluster.
...
Datastore Clusters are a collection of Datastores and use SDRS for placement of the .VMDK files on specific Datastores.
Storage Distributed Resource Scheduler (SDRS) can only be applied to Datastore Clusters, and is used to automate this process.
****
endif::[]
...
+
. From the left side panel, navigate to *Virtualization -> Virtual Machines*.
...
. Select *Specific Node*, which will then display the list of available nodes. Select the check the box next to a worker node and click *Migrate Virtual Machine*
...
You can follow https://issues.redhat.com/browse/VIRTSTRAT-97[VM Storage Migration] for more information.
====
...
. Once the migration is *Complete*, from the left side panel, navigate back to *Virtualization -> Virtual Machines*.
