...
+
.. Patch the existing *stress-test* deployment to use a node selector so that is runs on the node where at the VM is running.

+
We do this dynamically by:

... Getting all of the nodes

+
`oc get nodes -o custom-columns=":metadata.name" --no-headers`

... Grep the specific node where the *dynamic-schedule-vm-1* VM is running

+
`grep $(oc get vmi dynamic-schedule-vm-1 -n dynamic-schedule -o json | jq -r '.status.nodeName')`
