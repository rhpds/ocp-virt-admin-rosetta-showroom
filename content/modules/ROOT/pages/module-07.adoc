...
[NOTE]
=====
Using `-w` to apply a watch, you will see the `Migration` progress through a number of `PHASEs` from `Scheduling` through to `Succeeded`.
=====
...
. When the `Plan` is created, you will be taken to the `Plan details` page when you can monitor the status and progress of your migration. 
