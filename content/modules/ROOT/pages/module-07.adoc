...
****
OpenShift does live migration of storage and compute, as does VMware.

Storage migration in VMware is already built in.

This feature provides similar functionality and benefit as Cross-vCenter vMotion.
****
endif::[]
...
. Start the source Virtual Machine
+
...
+
It is expected that the DV and PVC are *PendingPopulation* and *Pending* until the migration starts.
...
+
You will notice that the *VirtualMachine* and *VirtualMachineInstance* have a unique *PHASE*, *WaitingForReceiver* and *WaitingForSync*.

+
This indicates the *VirtualMachine* is waiting for the migration to start as a receiver and the *VirtualMachineInstance* is waiting for data to be synchronized from the source *VirtualMachine*.
...
[NOTE]
=====
The *migrationID* field in the YAML above does not refer to anything specific, but must be unique across all migrations. You may use any unique text in that field, but both the source and destination *VirtualMachineInstanceMigration* objects must use the same *migrationID* for a migration.
=====
+
...
+
[loweralpha]
.. Find the IP address of the *virt-synchronization-controller* lease holder:
...
[IMPORTANT]
====
The *IP address* in your cluster *will be different*! Make sure you use the correct IP address from the output!
====
...
+
. Take the *pod* *IP address* from the previous command output (*10.232.1.136* in the example) and replace *<leader_sync_controller_ip>* in the *VirtualMachineInstanceMigration* YAML below. Take care that you do not remove the port *:9185*
...
+
The *VirtualMachine* will show it is *Migrating*.
...
+
From the left side panel, navigate to *Virtualization -> Virtual Machines*. Under *All projects*, select the *vm-live-migration-source* namespace and click on the Virtual Machine named *vm-migration-ns-ns-live*.

+
On the *Virtual machine details* page, you will see the *Source* VM is *Running*.
...
+
On the *Virtual machine details* page, you will see the *Destination* VM. If the migration has not started, it will have a status of *WaitingForReceiver*.
...
[IMPORTANT]
====
Stop the *vm-migration-ns-ns-live* VM using *virtctl* from your *Terminal* window to ensure you have enough resources for the next labs.
